...
void plotMCFMXsec_Z2jet(std::string file="Z_2jet_tota_CT10_mcfm.root")
{
  const double fBinsPt[]= {30,40,55,75,105,150,210,315,450};
  ...
  TH1D* h_diff_mcfm_jetpt02 = new TH1D("h_diff_mcfm_jetpt02", "",
				       nPtBins, fBinsPt);
  TH1D* h_diff_mcfm_jety02 = new TH1D("h_diff_mcfm_jety02", "",
				       nYBins, fBinsY);
  ...
  for(int i=1; i<= nYBins; i++)
    {
      h_diff_mcfm_jety02->SetBinContent(i, h_mcfm_original_y->GetBinContent(i));
      h_diff_mcfm_jety02->SetBinError(i,h_mcfm_original_y->GetBinError(i));

      total_y_xsec += h_diff_mcfm_jety02->GetBinContent(i)*
	h_diff_mcfm_jety02->GetBinWidth(i);

      cout << "Differential cross section at "
	   << h_diff_mcfm_jety02->GetBinLowEdge(i) 
	   << "~"
	   << h_diff_mcfm_jety02->GetBinLowEdge(i+1) 
	   << " = " 
	   << h_diff_mcfm_jety02->GetBinContent(i)
	   << " +- " 
	   << h_diff_mcfm_jety02->GetBinError(i) <<  " fb" << endl;
    }
...
//       cout << "Integrated cross section = " << mcfm_diff << " fb" << endl;
	...
	mcfm_diff_err += pow(h_mcfm_original_pt->GetBinError(ib),2);
      ...
      h_diff_mcfm_jetpt02->SetBinContent(i,mcfm_diff);
      h_diff_mcfm_jetpt02->SetBinError(i,mcfm_diff_err);

      total_pt_xsec += h_diff_mcfm_jetpt02->GetBinContent(i)*
	h_diff_mcfm_jetpt02->GetBinWidth(i);
  ...
  for(int i=1; i<= nPtBins; i++)
    {
      ...
      cout << "Differential cross section at "
	   << h_diff_mcfm_jetpt02->GetBinLowEdge(i) 
	   << "~"
	   << h_diff_mcfm_jetpt02->GetBinLowEdge(i+1) 
	   << " = " 
	   << h_diff_mcfm_jetpt02->GetBinContent(i)
	   << " +- " 
	   << h_diff_mcfm_jetpt02->GetBinError(i) <<  " fb/GeV" << endl;
    }
