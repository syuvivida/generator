...
void plotMCFMXsec_Z3jet(std::string file="Z_3jet_tota_CT10_mcfm.root")
{
  const double fBinsPt[]= {30,40,55,75,105,150,300};
  ...
  TH1D* h_diff_mcfm_jetpt03 = new TH1D("h_diff_mcfm_jetpt03", "",
				       nPtBins, fBinsPt);
  TH1D* h_diff_mcfm_jety03 = new TH1D("h_diff_mcfm_jety03", "",
				       nYBins, fBinsY);
  ...
  h_mcfm_original_pt =  (TH1F*)(fmcfm->Get("id9"));
  h_mcfm_original_pt -> SetName("h_mcfm_original_pt");

  h_mcfm_original_y =  (TH1F*)(fmcfm->Get("id10"));
  h_mcfm_original_y -> SetName("h_mcfm_original_y");

  double total_y_xsec = 0;
  for(int i=1; i<= nYBins; i++)
    {
      h_diff_mcfm_jety03->SetBinContent(i, h_mcfm_original_y->GetBinContent(i));
      h_diff_mcfm_jety03->SetBinError(i,h_mcfm_original_y->GetBinError(i));

      total_y_xsec += h_diff_mcfm_jety03->GetBinContent(i)*
	h_diff_mcfm_jety03->GetBinWidth(i);

      cout << "Differential cross section at "
	   << h_diff_mcfm_jety03->GetBinLowEdge(i) 
	   << "~"
	   << h_diff_mcfm_jety03->GetBinLowEdge(i+1) 
	   << " = " 
	   << h_diff_mcfm_jety03->GetBinContent(i)
	   << " +- " 
	   << h_diff_mcfm_jety03->GetBinError(i) <<  " fb" << endl;
    }
...
//       cout << "Integrated cross section = " << mcfm_diff << " fb" << endl;
	...
	mcfm_diff_err += pow(h_mcfm_original_pt->GetBinError(ib),2);
      ...
      h_diff_mcfm_jetpt03->SetBinContent(i,mcfm_diff);
      h_diff_mcfm_jetpt03->SetBinError(i,mcfm_diff_err);

      total_pt_xsec += h_diff_mcfm_jetpt03->GetBinContent(i)*
	h_diff_mcfm_jetpt03->GetBinWidth(i);
  ...
  for(int i=1; i<= nPtBins; i++)
    {
      ...
      cout << "Differential cross section at "
	   << h_diff_mcfm_jetpt03->GetBinLowEdge(i) 
	   << "~"
	   << h_diff_mcfm_jetpt03->GetBinLowEdge(i+1) 
	   << " = " 
	   << h_diff_mcfm_jetpt03->GetBinContent(i)
	   << " +- " 
	   << h_diff_mcfm_jetpt03->GetBinError(i) <<  " fb/GeV" << endl;
    }
  ...
  TFile* outFile = new TFile(Form("mcfm3jetXsec_%s",file.data()),"recreate");       
  
  h_diff_mcfm_jetpt03->Write();
  h_diff_mcfm_jety03->Write();
